...
class HistoryScreen extends StatefulWidget {
  ...
  const HistoryScreen({super.key, required this.sensorId});
...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  Future<List<Measurement>> fetchHistory(String sensorId) async {
    ...
    final String url = 'http://$SERVER_IP:3000/api/history?sensorId=$sensorId&limit=100';
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Falló la carga del historial. Código: ${response.statusCode}');
    }
  }
  ...
  Widget buildLineChart(List<Measurement> historyData) {
    ...
    final double intervaloEjeX = (reversedData.length / 5).floorToDouble().clamp(1, 100).toDouble();
                        ...
                        getTitlesWidget: (value, meta) {
                            ...
                            if (index >= 0 && index < reversedData.length) {
                                ...
                                return SideTitleWidget(
                                    ...
                                    space: 8.0,
                                    child: Text('${time.hour}:${time.minute}', style: const TextStyle(fontSize: 10)),
                                );
                            }
                            return const Text('');
                        },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Histórico de ${widget.sensorId}'),
      ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: buildLineChart(historyData), // Llama al helper del gráfico
                  ),
                  
                  const Padding(
                    padding: EdgeInsets.symmetric(horizontal: 16.0, vertical: 8.0),
                    child: Text('Detalles del Historial (Más Reciente Primero):', 
                                style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  ),

                  // LISTADO COMPLETO DEL HISTORIAL
                  // historyData ya está ordenado del más nuevo al más viejo por la API
                  ...historyData.map((m) => ListTile(
                    title: Text('${m.temperatureC.toStringAsFixed(2)} °C'),
                    subtitle: Text('Fecha: ${m.timestamp}'),
                    trailing: Text('Pila: ${m.voltageV.toStringAsFixed(2)} V'),
                  )).toList(),
                ],
              ),
            );
